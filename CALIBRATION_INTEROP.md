...
Tonight's full-rig export was found at `/media/zerospace/SABRENT1TB/0918_asteria/test_003/camera.calib` (modified September 18, 2026, 22:18:39 EDT). It contains 25 cameras and one frame per camera. All 25 camera-name MAC addresses were matched against live PySpin transport-layer `GevDeviceMACAddress` / `DeviceSerialNumber` readings. Captury's numeric serials in this file exceed PySpin's by 33,554,432; the importer does **not** assume that arithmetic conversion. It maps by verified MAC and retains the original Captury serial in provenance. The normalized seed is `live-sessions/captury-bootstrap/20260918-test_003.json`. Identity/format validation does not establish geometric accuracy.

The UI's **Load calibration…** accepts `.calib` directly while cameras are connected. It gets native resolution and the MAC identity map from the live source, imports through `read_captury` / `load_seed`, and checks the exact roster before switching sessions. Missing/ambiguous MAC matches or mismatched image sizes fail before disconnecting the current session. Animated files prompt for an explicit frame. The imported calibration starts a fresh seeded session, keeps lens parameters fixed for pose refinement, and is saved with its source hash and identity mapping in `bootstrap.json`.
